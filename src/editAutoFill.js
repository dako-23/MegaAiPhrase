(function () {
    ...
    async function run() {
        ...
        for (let i = 0; i < 40; i++) {
            ...
            // ако стойността вече е нашата – готово
            if (input.value === current.phrase) {
                console.log('[MP] Phrase confirmed in input');
                break;
            }

            // force set
            setNativeValue(input, current.phrase);
            console.log('[MP] Trying to set phrase, attempt', i + 1);
